...
public class MessagingFactory extends ClientEntity implements IAmqpConnection {
    ...
    CompletableFuture<Void> sendSecurityToken(String sasTokenAudienceUri) {
        ...
        return tokenFuture.thenComposeAsync((t) -> {
            ...
            CompletableFuture<Void> sendTokenFuture = this.cbsLinkCreationFuture.thenComposeAsync((v) -> CommonRequestResponseOperations.sendCBSTokenAsync(this.cbsLink, Util.adjustServerTimeout(this.clientSettings.getOperationTimeout()), generatedSecurityToken), MessagingFactory.INTERNAL_THREAD_POOL);
